...
int corpus_load(const char *dir, corpus_t *out){
    ...
    while((e=readdir(d))){ if(!ends_bin(e->d_name)) continue;
        ...
        if(out->n==cap){ body_t *nv=realloc(out->v,(size_t)cap*2*sizeof(body_t)); if(!nv){ fclose(f); break; } out->v=nv; cap*=2; }
        body_t *b=&out->v[out->n]; b->buf=malloc(sz); if(!b->buf){ fclose(f); continue; }
        b->len=fread(b->buf,1,sz,f); fclose(f);
